...
module.exports = {
  ...
  langs: {
    en: {
      ...
      botAddedMessage: "𝐓𝐇𝐀𝐍𝐊𝐒 𝐅𝐎𝐑 𝐀𝐃𝐃𝐈𝐍𝐆 𝐌𝐄 𝐓𝐎 𝐘𝐎𝐔𝐑 𝐈𝐍𝐒𝐓𝐀𝐆𝐑𝐀𝐌 𝐂𝐇𝐀𝐓!",
    },
  },

  onStart: async ({ threadsData, message, event, api, usersData, getLang }) => {
    // Check for Instagram participant addition events (Adjust based on your bot handler framework)
    if (
      event.logMessageType !== "log:subscribe" &&
      event.type !== "instagram_participant_added"
    )
      return;
    ...
    if (
      !threadData ||
      (threadData.settings && threadData.settings.sendWelcomeMessage === false)
    )
      return;

    const addedMembers =
      event.logMessageData?.addedParticipants || event.addedUsers || [];
    const threadName = threadData.threadName || "our Instagram chat";
    const prefix = global.utils?.getPrefix(threadID) || "!";
    const inviterID = event.author || event.senderID;

    for (const user of addedMembers) {
      const userID = user.userFbId || user.id;
      ...
      const userName = user.fullName || user.name || "Instagram User";
      const inviterName = await usersData
        .getName(inviterID)
        .catch(() => "Someone");
      const memberCount = event.participantIDs?.length || 2;
      ...
      try {
        ...
        console.error("Instagram welcome image creation failed:", err);
      }
...
async function createWelcomeCard({
  ...
  ctx.save();
  ctx.shadowColor = "rgba(80,160,255,0.28)";
  ctx.shadowBlur = 22;
  ...
  ctx.lineWidth = 3.5;
  ctx.beginPath();
  ctx.arc(leftCX, avatarY, avatarR + 8, 0, Math.PI * 2);
  ctx.stroke();
  ctx.restore();
  ...
  ctx.lineWidth = 1.5;
  ctx.beginPath();
  ctx.arc(leftCX, avatarY, avatarR + 17, 0, Math.PI * 2);
  ctx.stroke();
  ...
  if (newUserImg) {
    ...
    ctx.beginPath();
    ctx.arc(leftCX, avatarY, avatarR, 0, Math.PI * 2);
    ctx.fill();
    ctx.save();
    ctx.textAlign = "center";
    ctx.textBaseline = "middle";
    ctx.font = `${Math.round(avatarR * 0.7)}px Arial`;
...

  {
    ...
    ctx.lineWidth = 1;
    ctx.beginPath();
    ctx.moveTo(leftCX - dw / 2, dy);
    ctx.lineTo(leftCX + dw / 2, dy);
    ctx.stroke();
  }
  ...
  ctx.shadowBlur = 12;
  ctx.fillText("Welcome To Instagram", rX, 90);
  ctx.restore();
  ...
  if (inviterImg) {
    ...
    ctx.lineWidth = 2.5;
    ctx.beginPath();
    ctx.arc(invCX, invCY, invAvR + 4, 0, Math.PI * 2);
    ctx.stroke();
    ctx.restore();
    ...
    ctx.fillStyle = "#161628";
    ctx.beginPath();
    ctx.arc(invCX, invCY, invAvR, 0, Math.PI * 2);
    ctx.fill();
    ctx.save();
    ctx.textAlign = "center";
    ctx.textBaseline = "middle";
...

  {
...

    {
      ...
      ctx.lineWidth = 1;
      ctx.beginPath();
      ctx.moveTo(rX, blockY - 10);
      ctx.lineTo(rRight, blockY - 10);
      ctx.stroke();
    }
    ...
    ctx.fillStyle = pillG;
    roundRect(ctx, rX, blockY, rRight - rX, blockH, 14);
    ctx.fill();
    ...
    ctx.shadowBlur = 16;
    ctx.fillText("Powered By CEO HABIB", cx, cy + 8);
    ctx.restore();
  }
