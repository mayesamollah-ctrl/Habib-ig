...
module.exports = {
  ...
  onStart: async ({ threadsData, message, event, api, usersData, getLang }) => {
    ...
    if (event.logMessageType !== "log:subscribe" && event.type !== "instagram_participant_added") return;

    const { threadID } = event;  
    const threadData = await threadsData.get(threadID);  
    if (!threadData || (threadData.settings && threadData.settings.sendWelcomeMessage === false)) return;  

    const addedMembers = event.logMessageData?.addedParticipants || event.addedUsers || [];  
