...
module.exports = {
  ...
  async run(bot, data) {
    try {
      if (!config.LOG_EVENTS.event && !config.LOG_EVENTS.disableAll === false)
        return;
      ...
      for (const participant of addedParticipants) {
        ...
        const fullName =
          participant.fullName || participant.name || `User ${userId}`;

        logger.info(
          `New member joined thread ${threadID}: ${fullName} (${userId})`,
        );
