...
module.exports = {
  config: {
    ...
    description:
      "Unsend bot messages when an admin or developer reacts with an anger emoji",
  },

  async run(bot, event) {
    try {
      const { senderID, threadId, reaction, targetMessageId, reactionStatus } =
        event;
      ...
      const isBotMessage = sentMessages.some(
        (msg) => msg.itemId === targetMessageId,
      );
      if (!isBotMessage) return;

      await bot.api.unsendMessage(threadId, targetMessageId);
      logger.info("Message unsent via anger reaction", {
        senderID,
        threadId,
        targetMessageId,
      });
