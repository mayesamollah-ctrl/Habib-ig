...
module.exports = {
  ...
  ACCOUNT_PASSWORD:
    process.env.ACCOUNT_PASSWORD || c.instagramAccount?.password || "",
  ...
  MONGODB_DATABASE:
    ...
    c.database?.mongodbDatabase ||
    "instagram_bot",
  DB_AUTO_SYNC_WHEN_START: c.database?.autoSyncWhenStart ?? false,
  DB_AUTO_REFRESH_THREAD_FIRST:
    c.database?.autoRefreshThreadInfoFirstTime ?? true,
  ...
  RESTART_LISTEN_MQTT: {
    ...
    delayAfterStopListening:
      c.restartListenMqtt?.delayAfterStopListening ?? 2000,
    logNoti: c.restartListenMqtt?.logNoti ?? true,
  },

  // ── Notifications on MQTT error ───────────────────────────────────────
  NOTI_MQTT_ERROR: {
    telegram: c.notiWhenListenMqttError?.telegram || {
      enable: false,
      botToken: "",
      chatId: "",
    },
    discordHook: c.notiWhenListenMqttError?.discordHook || {
      enable: false,
      webhookUrl: "",
    },
  },
  ...
  HIDE_NOTI: {
    ...
    needRoleToUseCmdOnReply:
      c.hideNotiMessage?.needRoleToUseCmdOnReply ?? false,
    needRoleToUseCmdOnReaction:
      c.hideNotiMessage?.needRoleToUseCmdOnReaction ?? false,
  },
