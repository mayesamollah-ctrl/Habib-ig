module.exports = {
  ...
  async run({ api, event, args, config, database, logger }) {
    ...
    if (action === "ban")
      return this.banThread(api, event, args, database, logger);

    if (action === "unban")
      return this.unbanThread(api, event, args, database, logger);

    if (action === "prefix") {
      if (!args[1])
        return api.sendMessage(
          "⚠️ Usage: thread prefix <value>",
          event.threadId,
        );
    ...
    if (action === "resetprefix")
      return this.resetPrefix(api, event, database, config);
  ...
  banThread(api, event, args, database, logger) {
    ...
    if (td.settings?.banned) {
      return api.sendMessage(
        `ℹ️ Thread ${targetId} is already banned.`,
        event.threadId,
      );
    }

    database.setThreadData(targetId, {
      settings: { ...td.settings, banned: true },
    });
    database.save();
    logger.info(`Thread ${targetId} banned by ${event.senderID}`);
    return api.sendMessage(
      `🚫 Thread ${targetId} has been banned from using the bot.`,
      event.threadId,
    );
  },
  ...
  unbanThread(api, event, args, database, logger) {
    ...
    if (!td.settings?.banned) {
      return api.sendMessage(
        `ℹ️ Thread ${targetId} is not currently banned.`,
        event.threadId,
      );
    }
    ...
    return api.sendMessage(
      `✅ Thread ${targetId} has been unbanned.`,
      event.threadId,
    );
  },
  ...
  listBanned(api, event, database) {
    ...
    if (banned.length === 0)
      return api.sendMessage(
        "ℹ️ No threads are currently banned.",
        event.threadId,
      );
    return api.sendMessage(
      `🚫 Banned Threads (${banned.length}):\n\n${banned.join("\n")}`,
      event.threadId,
    );
  },
  ...
  setPrefix(api, event, value, database, config) {
    if (value.length > 10)
      return api.sendMessage(
        "⚠️ Prefix must be 10 characters or less.",
        event.threadId,
      );
    database.setThreadData(event.threadId, { prefix: value });
    database.save();
    return api.sendMessage(
      `✅ Thread prefix set to: ${value}\n\nGlobal prefix is still: ${config.PREFIX}`,
      event.threadId,
    );
  },
  ...
  resetPrefix(api, event, database, config) {
    ...
    database.save();
    return api.sendMessage(
      `✅ Thread prefix reset to global default: ${config.PREFIX}`,
      event.threadId,
    );
  },
};
