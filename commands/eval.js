  ...
  async run({ api, event, args, logger, config }) {
    ...
    try {
      ...
      try {
        ...
        return api.sendMessage(
          "❌ vm2 library not installed. Ask admin to install it.",
          event.threadId,
        );
      }
      ...
      const text =
        result !== undefined ? output(result) : "✅ Executed (no return value)";
      const trimmed =
      ...
      const errText = (error.stack || error.message || String(error)).substring(
        0,
        1997,
      );
