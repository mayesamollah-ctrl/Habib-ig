module.exports = {
  ...
  toggleMode(api, event, args, action, ConfigManager) {
    ...
    if (!["user", "thread"].includes(scope)) {
      return api.sendMessage(
        "⚠️ Specify scope: whitelist on/off user|thread",
        event.threadId,
      );
    }
  ...
  modifyEntry(api, event, args, action, ConfigManager) {
    ...
    if (!["user", "thread"].includes(scope)) {
      return api.sendMessage(
        `⚠️ Specify scope: whitelist ${action} user|thread <id>`,
        event.threadId,
      );
    }
    if (!id || !/^\d+$/.test(id)) {
    ...
    if (scope === "user") {
      ...
      if (action === "add") {
        if (list.includes(id))
          return api.sendMessage(
            `ℹ️ User ${id} is already whitelisted.`,
            event.threadId,
          );
        list.push(id);
      } else {
        ...
        if (idx === -1)
          return api.sendMessage(
            `ℹ️ User ${id} is not in the whitelist.`,
            event.threadId,
          );
        list.splice(idx, 1);
      }
      ...
      if (!cfg.whiteListModeThread)
        cfg.whiteListModeThread = { enable: false, whiteListThreadIds: [] };
      if (!cfg.whiteListModeThread.whiteListThreadIds)
        cfg.whiteListModeThread.whiteListThreadIds = [];
      const list = cfg.whiteListModeThread.whiteListThreadIds;

      if (action === "add") {
        if (list.includes(id))
          return api.sendMessage(
            `ℹ️ Thread ${id} is already whitelisted.`,
            event.threadId,
          );
        list.push(id);
      } else {
        ...
        if (idx === -1)
          return api.sendMessage(
            `ℹ️ Thread ${id} is not in the whitelist.`,
            event.threadId,
          );
        list.splice(idx, 1);
      }
    ...
    return api.sendMessage(
      `✅ ${scope.charAt(0).toUpperCase() + scope.slice(1)} ${id} ${verb} the whitelist.`,
      event.threadId,
    );
  },
  ...
  listEntries(api, event, args, ConfigManager) {
    ...
    if (scope === "thread") {
      const list = cfg.whiteListModeThread?.whiteListThreadIds || [];
      if (list.length === 0)
        return api.sendMessage("ℹ️ Thread whitelist is empty.", event.threadId);
      return api.sendMessage(
        `📋 Whitelisted Threads (${list.length}):\n\n${list.join("\n")}`,
        event.threadId,
      );
    }
    ...
    const list = cfg.whiteListMode?.whiteListIds || [];
    if (list.length === 0)
      return api.sendMessage("ℹ️ User whitelist is empty.", event.threadId);
    return api.sendMessage(
      `📋 Whitelisted Users (${list.length}):\n\n${list.join("\n")}`,
      event.threadId,
    );
  },
};
