module.exports = {
  ...
  async addAdmin(
    ...
    if (ConfigManager.isAdmin(userIdToAdd)) {
      return api.sendMessage(
        `❌ User ${userIdToAdd} is already an admin!`,
        event.threadId,
      );
    }
    ...
    if (userIdToAdd === ConfigManager.getDeveloper()) {
      return api.sendMessage(
        `ℹ️ User ${userIdToAdd} is the developer (already has highest permissions).`,
        event.threadId,
      );
    }
    ...
    try {
      ...
      if (!userInfo) {
        return api.sendMessage(
          `❌ Could not find user with ID: ${userIdToAdd}`,
          event.threadId,
        );
      }
  ...
  async removeAdmin(
    ...
    if (!ConfigManager.isAdmin(userIdToRemove)) {
      return api.sendMessage(
        `❌ User ${userIdToRemove} is not an admin!`,
        event.threadId,
      );
    }
