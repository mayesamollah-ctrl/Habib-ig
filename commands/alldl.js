...
module.exports = {
  config: {
    ...
    description:
      "Download videos from Facebook, Instagram, TikTok, YouTube via link",
    usage: "alldl <url>",
    cooldown: 10,
    role: 0,
    author: "Neoaz",
  ...
  async run({ api, event, args, logger }) {
    ...
    try {
      const res = await axios.get(
        `https://neoaz.is-a.dev/api/download?url=${encodeURIComponent(url)}`,
        {
          timeout: 30000,
        },
      );
      ...
      const videoUrl =
        res.data?.video?.directUrl || res.data?.video?.downloadUrl;
