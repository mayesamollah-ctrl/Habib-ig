  ...
  try {
    ...
    if (!mod.config || !mod.config.name) {
      return {
        success: false,
        name: fileName,
        error: new Error("Missing config.name in module"),
      };
    }
    ...
    if (Array.isArray(mod.config.aliases)) {
      mod.config.aliases.forEach((alias) =>
        commandLoader.commands.set(alias, mod),
      );
    }
...
module.exports = {
  ...
  async run({ api, event, args, bot, logger }) {
    ...
    if (sub === "load") {
      ...
      if (result.success) {
        return api.sendMessage(
          `✅ Command "${result.name}" loaded successfully.`,
          event.threadId,
        );
      } else {
        return api.sendMessage(
    ...
    if (sub === "loadall") {
      ...
      let msg = "";
      if (success.length)
        msg += `✅ Loaded (${success.length}): ${success.join(", ")}`;
      if (failed.length)
    ...
    if (sub === "unload") {
      ...
      if (!name) {
        return api.sendMessage(
          "⚠️ Usage: cmd unload <command name>",
          event.threadId,
        );
      }
      ...
      if (result.success) {
        return api.sendMessage(
          `✅ Command "${result.name}" unloaded successfully.`,
          event.threadId,
        );
      } else {
        return api.sendMessage(
    ...
    if (sub === "install") {
      ...
      if (isURL(urlOrCode)) {
        ...
        if (!fileName || !fileName.endsWith(".js")) {
          return api.sendMessage(
            "⚠️ Please provide a filename ending in .js",
            event.threadId,
          );
        }
        ...
        if (urlOrCode.endsWith(".js") && !isURL(urlOrCode)) {
          ...
          const bodyAfterSub = event.body
            .slice(event.body.toLowerCase().indexOf("install") + 7)
            .trim();
          ...
          const bodyAfterSub = event.body
            .slice(event.body.toLowerCase().indexOf("install") + 7)
            .trim();
          rawCode = bodyAfterSub
            .slice(0, bodyAfterSub.lastIndexOf(fileName))
            .trim();
        } else {
          return api.sendMessage(
      ...
      if (!rawCode || !rawCode.trim()) {
        return api.sendMessage(
          "❌ No command code found to install.",
          event.threadId,
        );
      }
