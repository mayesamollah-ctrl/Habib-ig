...
Export your Instagram session cookies in **Netscape format** using a browser extension (e.g. _Get cookies.txt LOCALLY_) and save them to `account.txt`.
...
module.exports = {
  ...
  async run({ api, event, bot }) {
    ...
    await api.sendMessage(
      `User ${leftUserId} has left the chat. Goodbye!`,
      threadId,
    );
  },
};
