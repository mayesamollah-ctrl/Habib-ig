...
class InstagramBot {
  ...
  startHealthServer() {
    const port = parseInt(
      process.env.PORT || config.DASHBOARD_PORT || 3000,
      10,
    );
  ...
  async loadAndLogin() {
    ...
    const cookieContent = hasCookieFile
      ? fs.readFileSync(config.ACCOUNT_FILE, "utf-8")
      : "";
    const hasValidCookies =
      hasCookieFile && this._hasValidCookies(cookieContent);
    ...
    if (
      hasCredentials &&
      config.AUTO_REFRESH_FBSTATE &&
      config.INTERVAL_GET_NEW_COOKIE
    ) {
  ...
  startListening() {
    ...
    this.ig.listen((err, event) => {
      if (err) {
        ...
        const isAuthError = /not authorized|login_required|unauthorized/i.test(
          msg,
        );
        if (isAuthError) {
          logger.error(
            "Session expired or invalid. Update account.txt or credentials in config.",
          );
  ...
  _scheduleMqttRestart() {
    ...
    const { timeRestart, delayAfterStopListening, logNoti } =
      config.RESTART_LISTEN_MQTT;
  ...
  async handleThreadEvent(event) {
    try {
      ...
      if (logType === "log:subscribe") {
        ...
        const botAdded = added.some(
          (p) => String(p.userFbId || p.userId || "") === String(this.userID),
        );
  ...
  async getThreadInfo(threadID) {
    ...
    try {
      ...
      this._threadInfoCache.set(String(threadID), {
        data: info,
        ts: Date.now(),
      });
      return info;
    ...
    return {
      ...
      getUserInfoByUsername: async (username) => {
        try {
          ...
          logger.error("Failed to get user info by username", {
            error: error.message,
            username,
          });
          throw error;
      ...
      sendPhotoFromUrl: async (threadID, url, opts = {}) => {
        try {
          ...
          logger.error("Failed to send photo from url", {
            error: error.message,
            threadID,
          });
          throw error;
      ...
      sendVideoFromUrl: async (threadID, url, opts = {}) => {
        try {
          ...
          logger.error("Failed to send video from url", {
            error: error.message,
            threadID,
          });
          throw error;
      ...
      sendVoiceFromUrl: async (threadID, url, opts = {}) => {
        try {
          ...
          logger.error("Failed to send voice from url", {
            error: error.message,
            threadID,
          });
          throw error;
      ...
      try {
        ...
        for (const reminder of due) {
          ...
          try {
            ...
            logger.warn("Could not deliver reminder", {
  ...
  _scheduleAutoUptime() {
    ...
    logger.info(
      `Auto-uptime ping to ${url} every ${config.AUTO_UPTIME_INTERVAL}s`,
    );
    setInterval(() => {
      axios.get(url).catch(() => {});
    }, intervalMs);
  ...
  _scheduleCookieRefresh() {
    ...
    logger.info(
      `Cookie auto-refresh scheduled every ${config.INTERVAL_GET_NEW_COOKIE} minutes.`,
    );
  ...
  async _sendMqttErrorNotification(errorMsg) {
    ...
    if (discordHook.enable && discordHook.webhookUrl) {
      ...
      for (const url of urls) {
        axios
          .post(url, { content: `⚠️ Bot MQTT error:\n${errorMsg}` })
          .catch(() => {});
      }
    }
  }
  ...
  scheduleReconnect() {
    ...
    logger.info(
      `Reconnecting in 5s (attempt ${this.reconnectAttempts}/${config.MAX_RECONNECT_ATTEMPTS})…`,
    );
    setTimeout(() => {
